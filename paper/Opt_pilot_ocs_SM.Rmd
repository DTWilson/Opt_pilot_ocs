...
First we set up our value function based on the elicited preference parameters $\bar{d}, \hat{d}$ and $n_*$:
...
As given in the paper, we use an exponential utility based on our value function, which itself has three attributes $n$ (the total sample size), $d$ (the change in mean outcome following the pilot and main trial programme), and $c$ (an indicator of the fixed cost difference between the control and experimental treatments). This utility is parameterised by $\rho$, which encodes the decision maker's attitude to risk.
...
p_u <- ggplot(to_plot_u, aes(rho, d_bar, z=dif)) + geom_contour(aes(colour=..level..), breaks=seq(2,20,2)) +
  theme_minimal() +
  xlab( expression(paste("Risk attitude, ", rho))) + 
  ylab( expression(paste("Cost of sampling, ", bar(d)))) 
direct.label(p_u, method = "bottom.pieces")
...
We need to change is the calculation of power at the pilot stage in our conditional expected utility, now needing to use the distribution of the pilot estimate conditional on the true main trial effect $\mu$:
