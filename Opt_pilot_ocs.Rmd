...
for(i in 1:nrow(df)){
  k <- get_ks(df$d_bar[i], d_hat)
  opt <- psoptim(x, exp_u_joint, k=k, rho=df$rho[i], mu_0=mu_0, sd_0=sd_0, sig=sig, rule=rule, 
                  lower = c(0,-5,0,0), upper= c(300,5,300,5), gr=exp_u_joint_grad)
              # method = "L-BFGS-B")#, control=list(maxit=1000))
  #opt <- genoud(exp_u_joint, 4, Domains = matrix(c(0,-5,0,0,300,5,300,5),ncol=2), print.level = 0, boundary.enforcement = 2, gradient.check = T, starting.values = x,
  #              gr=exp_u_joint_grad,
