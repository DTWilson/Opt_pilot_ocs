...
```{r, eval=T}
test_u <- function(mu, x, rho, mu_0, k)
{
  k_d <- k[1]; k_n <- k[2]; k_c <- k[3]
  go <- x > d
  if(rho > 0){
...
test_sim <- function(sig, rho, mu_0, sd_0, k)
{
  mu <- rnorm(1, mu_0, sd_0)
  x <- rnorm(1, mu, sqrt(2*sig^2/n))
  c(mu, x, test_u(mu, x, rho, mu_0, k))
}
...
N <- 1000000
us <- replicate(N, test_sim(sig, rho, mu_0, sd_0, k)[3])
...
$$
u(\mu, x_1, x_2) = 1 - e^{-\rho(k_d\mu + k_n (n_1+n_2))I[x_1>d_1, x_2>d_2] -\rho(k_n (n_1+n_2) + k_c)I[x_1 > d_1, x_2 < d_2] - \rho(k_n n_1 + k_c)I[x_1 < d_1]}.
$$
...
df_sen <- expand.grid(d_bar=seq(0.001, 0.05, length.out = 30), rho=seq(-5,5,length.out = 21))
