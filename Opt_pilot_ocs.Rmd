...
exp_u <- function(d, n, d_hat, d_bar, rho, sig, mu_0, sd_0)
{
  ...
  (1 - pnorm((d-mu_0)/sig_x)) * (1 - exp(-rho*k_n*n) *
    ...
    k_n*n + k_d*mu_0 + k_c*(pnorm((d-mu_0)/sig_x))
  }
}
...
df_sen <- expand.grid(d_bar=seq(0.001, 0.05, length.out = 20), rho=seq(-3,3,length.out = 21)[-11])
...
#ggplot(df_sen, aes(alpha, 1-cp, colour=1-d_bar)) + geom_point() + 
#  theme_minimal() + transition_states(rho)
```
