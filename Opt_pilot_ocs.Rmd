...
```{r}
# Example OC curve for a low powered pilot trial
alphas <- seq(0,1,0.001)
betas <- 1-sapply(alphas, function(a) power.t.test(n=30, delta=0.3, sig.level = a, alternative = "o")$power)

df <- data.frame(a = alphas, b =betas)

ggplot(df, aes(a,b)) + geom_line() + coord_fixed() +
  geom_point(data=df[1+0.05/0.001,], colour=cols[1]) +
  geom_point(data=df[nrow(df),], colour=cols[2]) +
  geom_point(data=df[nrow(df)-100,], colour=cols[3]) +
  theme_minimal()
```
...
We now need to define the component value functions $v_i$ and the corresponding weights $\lambda_i$. First consider the functions. We assume that the cost of sampling is constant, and so $v_1$ is linear, and similarly assume linearity in the value of change in outcome. Attribute $C$ only has two levels. We use identity value functions for all attributes, which will keep the notation simple. SO,

$$ v(n, C, \delta) = \lambda_n n + \lambda_c C + \lambda_d \delta,$$

We need to choose the scaling constants which will determine the realtive importance of each attribute. Given that $\lambda_n + \lambda_c + \lambda_d = 1$, we can deduce the parameter values given two sets of equivalence judgements. First, we ask for the change in outcome $\bar{d}$ that would be needed for us to judge
...
* $\lambda_d = 1/(1 + \hat{d} + \bar{d}/100)$,
* $\lambda_n = -\lambda_d \bar{d}/100$,
* $\lambda_c = \lambda_d \hat{d}$.
...
$$\lambda_n = 0.0260800, \lambda_d = 0.7668712, \lambda_c = 0.2070488.$$
...
This is all for the case $\rho > 0$, but the case $\rho < 0$ follows immediately. For the case $\rho = 0$ the utility function is the value function. So The expected value is:
...
$$
\mu_1 = \sigma_1^2(\frac{\mu_0}{\sigma_0^2} + \frac{xn}{2\sigma^2}), ~ \sigma_1^2 = 1/(\frac{1}{\sigma_0^2} + \frac{n}{2\sigma^2}).
$$
...
```{r, eval=T}
mu_0 <- 0; sd_0 <- 0.244949; sig <- 0.25; rho <- -2
...
test_sim <- function(sig, rho, mu_0, sd_0)
{
  mu <- rnorm(1, mu_0, sd_0)
  x <- rnorm(1, mu, sqrt(2*sig^2/n))
  c(mu, x, test_u(mu, x, rho, mu_0))
}

# Simulated expected utility
N <- 1000000
us <- replicate(N, test_sim(sig, rho, mu_0, sd_0)[3])
mean(us)
...
get_opt_alpha2 <- function(n, d_hat, d_bar, rho, sig, mu_m, mu_sd)
{
  # Evaluate on a set of points to get biundaries for the optimum
...
eval_scenario2 <- function(x)
{
  # Using the exact expected utility
  n <- x[1]; d_hat <- x[2]; d_bar <- x[3]; rho <- x[4]; sig <- x[5]; mu_m <- x[6]; mu_sd <- x[7]
  
  result <- get_opt_alpha2(n, d_hat, d_bar, rho, sig, mu_m, mu_sd)
  d <- result[1]; u <- result[2]
  alpha <- (n==0)*0 + (n!=0)*(1-pnorm(d*sqrt(n/(2*sig^2))))
  # Get conditional power at the original "mcid" of 0.2
  cp <- power(0.2, alpha, n, sig)
...
df$d_hat <- 0.142; df$d_bar <- 0.01; df$rho <- 0
df$sig <- 0.25; df$mu_m <- 0; df$mu_sd <- 0.244949

df <- cbind(df, t(apply(df, 1, eval_scenario2)))
...
The design which gives the maximum expected utility is a sample size of $n = 28$ and a type I error rate of $\alpha = 0.0104$. This gives a power of 0.75 to detect the original MCID of 0.2, and gives an unconditional probability of rejecting the null of 0.27. The null hypothesis will be rejected if the sample mean is greater than 0.156, or equivalently, if the posterior probability that $\mu > 0.142$ is greater than 0.5113. Note that the lowest expected utility is given by a sample size of $n = 0$. We can include a set-up cost into the value function `val_n()` (in units of sample size) to correct for this, leading to a minimum sample size for the trial to be better than doing nothing at all.
...
df_sen <- expand.grid(d_bar=seq(0.001, 0.05, length.out = 2), rho=seq(-3,3,length.out = 21))#[-11])
...
ggplot(df_sen, aes(alpha, 1-cp, colour=rho, alpha=1-d_bar)) + geom_point() + 
  theme_minimal()
...
ggplot(df2, aes(d_bar, val, linetype=var)) + geom_line() +
  theme_minimal() + 
...
```{r}
p1 <- ggplot(df_sen, aes(d_bar, cp, colour=rho, group=rho)) + geom_point() + geom_line()
p2 <- ggplot(df_sen, aes(rho, cp, colour=d_bar, group=d_bar)) + geom_point() + geom_line()
p3 <- ggplot(df_sen, aes(d_bar, alpha, colour=rho, group=rho)) + geom_point() + geom_line()
p4 <- ggplot(df_sen, aes(rho, alpha, colour=d_bar, group=d_bar)) + geom_point() + geom_line()
p5 <- ggplot(df_sen, aes(d_bar, n, colour=rho, group=rho)) + geom_point() + geom_line()
p6 <- ggplot(df_sen, aes(rho, n, colour=d_bar, group=d_bar)) + geom_point() + geom_line()
p7 <- ggplot(df_sen, aes(d_bar, d, colour=rho, group=rho)) + geom_point() + geom_line()
p8 <- ggplot(df_sen, aes(rho, d, colour=d_bar, group=d_bar)) + geom_point() + geom_line()
...
$$
It has not been possible to obtain a closed-form expression for the expectaed utility over the joint distribution of $\mu, x_1, x_2$. However, the expectation conditional on $\mu$ is
$$
\begin{aligned}
...
$$
where $f(\mu)$ is the normal prior density. Because we are integrating with a normal density weighting function, we can use Gauss-Hermite Quadrature (as implemented in the `fastGHQuad` package) to evaluate the integral efficiently and accurately. Specifically, Gauss-Hermite approximates integrals of the form
$$
...
Finally, we can optimise over the design parameters $n_1, d_1, n_2, d_2$. Note that we use a lower limit on $n_1 \geq 6$ here, felecting that fact that we will be running the pilot trial for reasons other than efficacy anyway (i.e. assessing recruitment etc), so the question is what level of test should be use if we maintain the default pilot sample size, and should we increase the pilot sample size (we are in a cluster RCT context here, so using at least 6 clusters per arm).

```{r, eval=F}
# Get optimal programmes for a range of risk aversions
k <- get_ks(0.01, 0.142)

df <- expand.grid(rho = rev(c(seq(-5,-0.001,0.2), 0.001, seq(0.001,5,0.2))),
                 d_bar = seq(0.002, 0.02, 0.002))
...
ggplot(df2[df2$var != "d",], aes(rho, val, colour=trial, linetype=var)) + geom_line() +
  theme_minimal() +
  scale_colour_manual(values=cols) +
  scale_y_continuous(breaks=seq(0,1,0.1)) + 
  transition_states(d_bar)

ggplot(df2[df2$var != "d",], aes(d_bar, val, colour=trial, linetype=var)) + geom_line() +
  theme_minimal() +
  scale_colour_manual(values=cols) +
  scale_y_continuous(breaks=seq(0,1,0.1)) + 
  transition_states(rho)
