...
Our presentation has been motivated  by the arguments around precision - i.e. that the small sample in a pilot trial will not provide enough information to allow reliable decision making. The other, and perhaps more persuasive, argument against assessing efficacy in pilots is that the effect size may differ. Recently, Ying et al. (2023) have reviewed pilot - main trial pairs and summarised the differences in their effect estimates. As we note in the discussion, in the extreme case where the amount of bias is unknown then it will be optimal to not assess efficacy in the pilot. Ying suggest the bias is typically not so large. By characterising the bias using a probability distribution we can ask - in what scenarios (as described by our uncertainty regarding the bias) will it be optimal to not assess effectiveness?

We can accomodate this bias easily enough - our expected utility calculations are mosty the same, but now when calculating the probability of pilot success conditional on $\mu$ (which denotes the true effect in the main trial) we will now have

$$
x_1 ~|~ \mu \sim N\left(\mu + b, \frac{2\sigma^2}{n} + s^2 \right),
$$

where the pilot effect size is now distributed as $\mu + \tau$, where $\tau$ is the (additive) bias and $\tau \sim N(b, s^2)$.

```{r}
exp_u_mu_bias <- function(mu, n1, d1, n2, d2, k, rho, mu_0, sd_0, sig, b_mean, b_var)
{
  pow1 <- 1-pnorm(d1, mu + b_mean, sqrt(2*sig^2/n1 + b_var))
  pow2 <- 1-pnorm(d2, mu, sqrt(2*sig^2/n2))
...
exp_u_joint_bias <- function(x, k, rho, mu_0, sd_0, sig, b_mean, b_var, rule, pen = FALSE)
{
  n1 <- x[1]; d1 <- x[2]; n2 <- x[3]; d2 <- x[4]
  u <- ghQuad(f=exp_u_mu_bias, rule=rule, n1=n1, d1=d1, n2=n2, d2=d2,
         k=k, rho=rho, mu_0=mu_0, sd_0=sd_0, sig=sig,
         b_mean=b_mean, b_var=b_var)/sqrt(pi)
...
# Mean bias has no influence so keep at 0 and just vary bias uncertainty
df <- expand.grid(b_var = seq(0.01, 0.7, length.out = 50)^2,
                  b_mean = c(0))
...
for(i in 1:nrow(df)){
  ...
  opt <- optim(x, exp_u_joint_bias, #gr = exp_u_joint_grad,
              lower = c(00,-40,0.0001,-40), upper = c(1000,40,1000,40), method = "L-BFGS-B",
              control = list(factr = 1e5),
              k=k, rho=rho, mu_0=mu_0, sd_0=sd_0, sig=sig, b_mean = df$b_mean[i], b_var = df$b_var[i],
              rule=rule, pen=F)
...
names(df) <- c("b_var", "b_mean", "n1", "d1", "n2", "d2", "a1", "b1", "a2", "b2", "u")
df
```

We find that the parameter $b$ has almost no influence on our results, suggesting we simply shift our criteria over to accommodate for this bias when testing at the pilot stage. The parameter $s^2$ is influential and in the way we would expect - as the uncertainty around the bias increases, the pilot information becomes less useful and we lower the pilot sample, increase the pilot type I error rate, and decrease the main trial type I error rate to compensate. We also see the main trial sample size decreasing. 
